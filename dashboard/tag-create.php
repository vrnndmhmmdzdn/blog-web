...
if (isset($_POST['submit'])) {
    $datas = ['tag_name' => $_POST['tag_name']];
    $result = $tags->Create($datas);
    if (gettype($result) == "string") {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                position: 'center-center',
                icon: 'success',
                title: 'Successfully added a new tag',
                showConfirmButton: false,
                timer: 2000
            });
            setTimeout(() => {
                                    ...
                                    <div class="card-body">
                                        <form class="flex flex-col gap-6" action="" method="post">
                                            <div>
                                                ...
                                                <input type="text" name="tag_name" id="input-label-with-helper-text" required
                                                    class="py-3 px-4 block w-full border-gray-200 rounded-md text-sm focus:border-blue-600 focus:ring-0 "
                                                    aria-describedby="hs-input-helper-text">
                                                <p class="text-sm  text-gray-500 mt-2" id="hs-input-helper-text">Tag will saparate article</p>
                                            </div>
