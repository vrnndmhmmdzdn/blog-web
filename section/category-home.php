...
$homes = new Home();
if ($check = $homes->Check('posts', 'post_category_id', $id)) {
    header("location: ../index.php");
}
$categoryTitle = $homes->Category($id);
$categoryContent = $homes->CategoryHome($id);
//var_dump($categoryContent);
//die;
$totalAuthors = $homes->GetAuthorCountForTag($id);
$likeTotal = $homes->LikeWatch('posts.post_like', $id);
$watchTotal = $homes->LikeWatch('posts.post_watched', $id);
                ...
                <div class="border border-gray-300 py-5 px-8">
                    <div class="w-full py-3 flex flex-col gap-5 border-b-2 mb-10" style="scrollbar-width: none;">
                        <div class="flex flex-row items-center gap-1">
                            ...
                            <span>/</span>
                            <p class="text-sm text-gray-700 font-normal hover:text-[#E20612]"><?= $categoryTitle[0]['category_name'] ?></p>

                        </div>
                        <div class="px-3 py-1 text-sm text-white font-normal bg-blue-600 w-fit rounded-sm">Category Title</div>
                        <h1 class="text-gray-800 text-[40px] font-bold leading-[48px]"><?= $categoryTitle[0]['category_name'] ?> </h1>
                        <div class="flex flex-row flex-wrap items-center justify-between">
                            <div class="flex flex-row items-center gap-3">
                                <div class="flex flex-row items-center gap-1 w-max"><i
                                    ...
                                    <span class="w-max text-gray-600 text-xs font-semibold"><?= count($categoryContent) . ' articles' ?></span>
                                </div>
                    ...
                    <div class="flex flex-col justify-center items-center gap-7">
                        <?php foreach ($categoryContent as $content): ?>
